...
internal abstract class GenerateKayanConfigTask : DefaultTask() {
    ...
    private fun loadGenerationInputsEither(): Either<KayanGradleError, GenerationInputs> = either {
        ...
        GenerationInputs(
            ...
            generatedTargetNames = generatedTargetNames.orNull.orEmpty().map(String::trim).filter(String::isNotEmpty).distinct(),
...
private fun GenerateKayanConfigTask.resolveDeclarationNullabilityEither(
    ...
    if (inputs.declarationMode == KayanDeclarationMode.OBJECT) {
        return emptyMap<ConfigDefinition, Boolean>().right()
    }
    ...
    return resolveTargetDeclarationNullabilityEither(
        schema = inputs.schema,
